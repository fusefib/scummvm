...
dispatch:
...

	{
		...
		case 0xFD:
			...
			goto fades;
		case 0xFE: {
			const uint16 count = readWord(pSrc);
			if (!channel._outerLoopCount) {
				channel._outerLoopCount = count;
				channel._outerLoopPtr = channel._pSrc + 3;
			}
			if (--channel._outerLoopCount)
		...
		case 0xFF: {
			...
			if (!channel._innerLoopCount) {
				channel._innerLoopCount = count;
				channel._innerLoopPtr = channel._pSrc + 3;
			}
			if (--channel._innerLoopCount)
