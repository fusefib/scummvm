...
#include "mads/nebular/isound_sections.h"
...
const uint kRexSoundFileCount = 9;

// These retain the existing MADS validation convention: MD5 of the first
// 8192 bytes of each overlay, rather than a whole-file hash.
const char *const kAdLibSoundMd5[] = {
...
const char *const kRolandSoundMd5[] = {
...
const char *const kRolandDemoSoundMd5[] = {
	"ad14e2a1c900287737b9f43f1d8c3fb2",
	nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
	"e2fafe292239be4afa2bf789bf4f496d",
};

const char *const kPcSpeakerSoundMd5[] = {
	"98f2ceb79d9df7b4d108a304b26368e6",
	"a21a46384a56c584d05c6fbca994e34e",
	"d0b4c48de6eb78eec0bf6369c66bf410",
	"aec7f555d36ecac9b4a82b681dfc1e9b",
	"5ac0d16c7d12afed72eb55a402b3186e",
	"80822a2886293c8fe30f51b8c211db29",
	"5e97dd51f5d595df3e7764b7706b6fb2",
	"c55841fa8d489811e4e1299c85d3ea98",
	"fbd438625a0b5a4faa3b045bc4e0f30f",
};
...
RexSoundManager::RexSoundManager(Audio::Mixer *mixer, bool &soundFlag,
		bool isDemo) :
		SoundManager(mixer, soundFlag),
		_midiDriver(nullptr),
		_selectedMidiDevice(0),
		_isDemo(isDemo),
		_soundFamily(kAdLibSound) {
	selectSoundDevice();
}

RexSoundManager::~RexSoundManager() {
	// RSound owns the timer callback but not the shared MIDI transport.
	// Destroy the section driver before closing the device.
	removeDriver();
	closeMidi();
}

void RexSoundManager::selectSoundDevice() {
	uint32 deviceFlags = MDT_ADLIB | MDT_MIDI | MDT_PREFER_MT32;
	if (!_isDemo)
		deviceFlags |= MDT_PCSPK;
	_selectedMidiDevice = MidiDriver::detectDevice(deviceFlags);

	const MusicType musicType =
			MidiDriver::getMusicType(_selectedMidiDevice);
	const bool configuredAsNativeMt32 =
			ConfMan.hasKey("native_mt32") && ConfMan.getBool("native_mt32");

	// Rex Roland resources contain custom MT-32 timbres and patch-memory
	// assignments. They are selected only for an MT-32-class device, or for a
	// generic external MIDI port explicitly marked as a true Roland MT-32.
	if (musicType == MT_PCSPK)
		_soundFamily = kPcSpeakerSound;
	else if (musicType == MT_MT32 ||
			(musicType != MT_ADLIB && configuredAsNativeMt32))
		_soundFamily = kRolandSound;
	else
		_soundFamily = kAdLibSound;

	_preferRoland = _soundFamily == kRolandSound;

	if (_soundFamily == kAdLibSound && musicType != MT_ADLIB &&
			musicType != MT_AUTO && musicType != MT_NULL) {
		warning("Rex Nebular MIDI output is not marked as MT-32-compatible; "
				"using the AdLib sound overlays");
	}
}

bool RexSoundManager::openMidi() {
	if (_midiDriver)
		return true;
	if (_soundFamily != kRolandSound)
		return false;

	_midiDriver = MidiDriver::createMidi(_selectedMidiDevice);
	if (!_midiDriver) {
		warning("Could not create Rex Nebular MT-32 MIDI driver");
		return false;
	}

	// RexRSound tracks the logical controller-7 state of all nine channels
	// and applies SoundManager's explicit 0-255 master volume itself. Leave
	// MidiDriver user-volume scaling disabled to avoid a second, global scale.
	const int result = _midiDriver->open();
	if (result) {
		warning("Could not open Rex Nebular MT-32 MIDI driver: %s",
...
void RexSoundManager::closeMidi() {
	if (!_midiDriver)
		return;
...
void RexSoundManager::validateSoundFiles(const char *prefix,
		const char *const *expectedMd5) {
	Common::File file;

	for (uint index = 0; index < kRexSoundFileCount; ++index) {
		if (!expectedMd5[index])
			continue;

		const Common::Path filename(Common::String::format(
				"%s.%03u", prefix, index + 1));
		if (!file.open(filename))
			error("Could not process - %s", filename.toString().c_str());

		const Common::String actualMd5 =
				Common::computeStreamMD5AsString(file, 8192);
		file.close();

		if (actualMd5 != expectedMd5[index])
			error("Invalid sound file - %s", filename.toString().c_str());
	}
}

void RexSoundManager::validate() {
	// Validate only the family selected by the standard ScummVM music device
	// setting. This permits installations that contain only the corresponding
	// original overlay family.
	if (_soundFamily == kRolandSound)
		validateSoundFiles("RSOUND",
				_isDemo ? kRolandDemoSoundMd5 : kRolandSoundMd5);
	else if (_soundFamily == kPcSpeakerSound)
		validateSoundFiles("ISOUND", kPcSpeakerSoundMd5);
	else
		validateSoundFiles("ASOUND", kAdLibSoundMd5);
}

void RexSoundManager::loadDriver(int sectionNumber) {
	if (_soundFamily == kRolandSound) {
		// Do not silently switch resource families after validation. An MT-32
		// device-open failure is reported explicitly so that missing hardware or
		// backend configuration cannot masquerade as an AdLib selection.
		if (!openMidi())
			error("Could not initialize the Rex Nebular MT-32 driver");

		// The demo has one numbered overlay for every gameplay section and a
		// separate opening overlay. Its game code still requests retail-style
		// section numbers, so map all numbered requests to RSOUND.001.
		if (_isDemo) {
			if (sectionNumber == 9)
				_driver = new RSoundDemo9(_mixer, _opl, _midiDriver);
			else
				_driver = new RSoundDemo1(_mixer, _opl, _midiDriver);
			return;
		}

		switch (sectionNumber) {
		case 1:
			_driver = new RSound1(_mixer, _opl, _midiDriver);
			break;
		case 2:
			_driver = new RSound2(_mixer, _opl, _midiDriver);
			break;
		case 3:
			_driver = new RSound3(_mixer, _opl, _midiDriver);
			break;
		case 4:
			_driver = new RSound4(_mixer, _opl, _midiDriver);
			break;
		case 5:
			_driver = new RSound5(_mixer, _opl, _midiDriver);
			break;
		case 6:
			_driver = new RSound6(_mixer, _opl, _midiDriver);
			break;
		case 7:
			_driver = new RSound7(_mixer, _opl, _midiDriver);
			break;
		case 8:
			_driver = new RSound8(_mixer, _opl, _midiDriver);
			break;
		case 9:
			_driver = new RSound9(_mixer, _opl, _midiDriver);
			break;
		default:
			_driver = nullptr;
			break;
	...
	if (_soundFamily == kPcSpeakerSound) {
		switch (sectionNumber) {
		case 1:
			_driver = new ISound1(_mixer, _opl);
			break;
		case 2:
			_driver = new ISound2(_mixer, _opl);
			break;
		case 3:
			_driver = new ISound3(_mixer, _opl);
			break;
		case 4:
			_driver = new ISound4(_mixer, _opl);
			break;
		case 5:
			_driver = new ISound5(_mixer, _opl);
			break;
		case 6:
			_driver = new ISound6(_mixer, _opl);
			break;
		case 7:
			_driver = new ISound7(_mixer, _opl);
			break;
		case 8:
			_driver = new ISound8(_mixer, _opl);
			break;
		case 9:
			_driver = new ISound9(_mixer, _opl);
			break;
		default:
			_driver = nullptr;
			break;
	...
	case 6:
		...
		break;
	case 7:
		_driver = new ASound7(_mixer, _opl);
		break;
	case 8:
		_driver = new ASound8(_mixer, _opl);
		break;
	case 9:
