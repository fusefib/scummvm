...
ISound::ISound(Audio::Mixer *mixer, OPL::OPL *opl,
		const Common::Path &filename) :
		ISound(mixer, opl, filename, readOverlayLayout(filename)) {
}

ISound::ISound(Audio::Mixer *mixer, OPL::OPL *opl,
		const Common::Path &filename, const OverlayLayout &layout) :
		SoundDriver(mixer, opl, filename,
				(int)layout.dataOffset, (int)layout.initializedDataSize),
		_speakerGate(false),
		_noiseEnabled(false),
