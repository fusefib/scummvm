...
SeekableAudioStream *makeMacSndMACEStream(Common::SeekableReadStream *stream,
		DisposeAfterUse::Flag disposeAfterUse, uint16 rate, uint32 frameCount,
		uint32 channels, MacSndMACEType type) {
	...
	if (frameCount > availablePackets) {
		warning("makeMacSndMACEStream(): MACE packet count %u exceeds available data %u",
			frameCount, availablePackets);
		frameCount = availablePackets;
	}

	if (frameCount > 0xffffffffU / (6 * channels)) {
		warning("makeMacSndMACEStream(): MACE sample count is too large");
		return nullptr;
	}

	const uint32 outputSize = frameCount * 6 * channels;
